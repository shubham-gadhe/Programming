//////////////////////////////////////////////////////////////////
//
//  File Name    : Calculate_Ticket_Price
//
//  Description  : This program accepts age from the user
//                 and calculates the ticket price based
//                 on the given age.
//
//  Example:
//      Input  : 25
//      Output : Your ticket price will be 900 rupees
//
//////////////////////////////////////////////////////////////////

//////////////////////////////////////////////////////////////////
//
//  Enhanced Version: Added proper error handling by processing
//                    invalid input using a user-defined macro
//                    and input validation.
//
//////////////////////////////////////////////////////////////////

/*
    Algorithm

    START

        Accept age as Age

        Call CalculateTicketPrice() by passing Age

        Receive the result

        If result is AGE_INVALID
            Display "Please enter positive Age"

        Otherwise
            Display the ticket price

    STOP
*/

#include<stdio.h>

// User-defined Macro
#define AGE_INVALID -1

int CalculateTicketPrice(int iAge)
{
    // Input Filter
    if(iAge < 0)
    {
        return AGE_INVALID;
    }

    if(iAge >= 0 && iAge <=5)
    {
        return 0;
    }
    else if (iAge >= 6 && iAge <=18)
    {
        return 500;
    }
    else if (iAge >= 19 && iAge <=50)
    {
        return 900;
    }
    else
    {
        return 400;
    } 
}

int main()
{
    int iValue = 0;
    int iRet = 0;

    printf("Please enter your age to calculate ticket prices : ");
    scanf("%d", &iValue);

    iRet = CalculateTicketPrice(iValue);

    if(iRet == AGE_INVALID)
    {
        printf("Please enter positive Age\n");
    }
    else
    {
        printf("Your ticket price will be %d rupees \n", iRet);
    }
    
    return 0;
}